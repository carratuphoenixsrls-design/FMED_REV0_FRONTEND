/* FMED REV0 · frontend ufficiale */

import { useEffect, useMemo, useDeferredValue, useState, useCallback, memo, lazy, Suspense } from "react";
import { SEDI_STANDARD_LIST, PERIODICITA_STANDARD, calcolaProssimaScadenza, etichetteDaDizionario, codiciDaDizionario, chiaveSedeCanonica, etichettaSedeCanonica, listaSediCanoniche } from "./fmedStandard.js";
...
import AlertMailPage from "./components/alerts/AlertMailPage.jsx";
...
function valoriDizionarioPrimari(dizionari, chiave, fallback = []) {
  const master = etichetteDaDizionario(dizionari, chiave, []);
  return master.length ? master : fallback;
}
...
// In locale usa 127.0.0.1:8000. In produzione usa l'URL configurato o Render.
...
const MRDB_APP_BUILD_LABEL = "FMED REV0";
...
function ttlCacheApiFmed(endpoint) {
  ...
  if (key.startsWith("/core/dizionari") || key.startsWith("/process-engine")) return 60_000;
...
function raggruppaCostiInterventi(interventi = [], keyFn) {
  ...
  (Array.isArray(interventi) ? interventi : []).forEach((intervento) => {
    ...
    mappa[chiave].totale += normalizzaNumeroCostoGlobale(intervento?.costo) +
    normalizzaNumeroCostoGlobale(intervento?.importo_extra);
...
function normalizeCategoria(categoria) {
  ...
  /* CATEGORIE ASSET ALIAS PULITI START */
  const chiaveCategoria = valore
    .toUpperCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/\s+/g, " ")
  ...
  const aliasCategorieAsset = {
    "ARREDO": "A",
    "A - ARREDO": "A",
    "ARREDO SANITARIO": "S",
    "S - ARREDO SANITARIO": "S",
    "ELETTROMEDICALE": "E",
    "E - ELETTROMEDICALE": "E",
    "GRUPPO ELETTROGENO": "G.E.",
    "G.E. - GRUPPO ELETTROGENO": "G.E.",
    "UPS - UPS - UNITA POWER SUPPLY": "UPS",
    "UPS - UNITA POWER SUPPLY": "UPS",
    "UNITA POWER SUPPLY": "UPS"
  };

  if (aliasCategorieAsset[chiaveCategoria]) {
    return aliasCategorieAsset[chiaveCategoria];
  }
  /* CATEGORIE ASSET ALIAS PULITI END */
...
function BottoneJobReport({
  ...
  if (!link.raw) {
    return <span className="fmed-style-muted-small">-</span>;
  }
  if (!link.apribile) {
    return <button type="button" title={link.raw} onClick={(e) => {
      e.stopPropagation();
      alert(`Percorso non apribile direttamente dal browser:\n\n${link.raw}`);
    }} className="fmed-style-doc-btn-disabled">
         Rif.
      </button>;
  }
  return <button type="button" title={link.url} onClick={(e) => {
    e.stopPropagation();
    window.open(link.url, "_blank");
  }} className="fmed-style-doc-btn">
       Apri
    </button>;
}
...
// Lo stile attivo è definito esclusivamente dai fogli CSS REV0.

function normalizzaInfrastrutturaDaApi(r = {}) {
  const sedeNormalizzata = normalizzaSedeDisplay(r.sede || r.Sede || "");
  return {
    ...r,
    id: r.id || r.ID || r.Id || "",
...
function AppNuovoCore({
  ...
  useEffect(() => {
    ...
    }document.body.dataset.theme = "light";document.documentElement.dataset.theme = "light";}, []);
  useEffect(() => {
    try {
    ...
    }}, [sidebarCompattata]);const [interventi, setInterventi] = useState([]);const [interventiIncludeStorico, setInterventiIncludeStorico] = useState(false);
  ...
  const [motivoChiusuraScadenze, setMotivoChiusuraScadenze] = useState("");
  const [infrastrutture, setInfrastrutture] = useState([]);
  ...
  const [cespiteRitornoOperazione, setCespiteRitornoOperazione] = useState(null);
  const [interventiCespite, setInterventiCespite] = useState([]);
  const [analisiCespite, setAnalisiCespite] = useState(null);
  const [analisiPredittivaAperta, setAnalisiPredittivaAperta] = useState(false);
  ...
  useEffect(() => {
    let attivo = true;
    Promise.all([ricaricaDizionariCoreFmed(), caricaJsonDaApi("/process-engine/catalogo", null)]).then(([, proc]) => {
      if (!attivo) return;
  ...
  const avviaProcessoGuidatoFmed = useCallback(async (codiceProcesso, datiIniziali = {}) => {
    ...
    // In questo modo chi torna subito all’elenco non lascia processi fantasma "IN CORSO".
  ...
  const caricaInfrastruttureOnDemand = useCallback(async ({
    ...
    try {
      ...
      const normalizzati = Array.isArray(dati) ? dati.map(normalizzaInfrastrutturaDaApi) : [];
  ...
  const apriOutlookAlertScadenze = useCallback(async (payload = {}) => {
    // Apriamo subito la nuova scheda esterna per evitare il blocco del browser durante la richiesta HTTP.
  ...
  useEffect(() => {
    ...
    chiamataApiAutenticataFmed(`/cespite/prossimo-codice?sede=${encodeURIComponent(formNuovoCespite.sede)}`, {
      method: "GET"
    }).then((data) => {
      const codiceGenerato = String(data?.prossimo_codice || "").trim();
      if (!annullato && codiceGenerato) {
        setFormNuovoCespite((prev) => prev.codicestrumento === codiceGenerato ? prev : {
          ...prev,
          codicestrumento: codiceGenerato
        });
      }
    }).catch((err) => {
      if (!annullato) console.error("Errore generazione codice backend:", err);
    }).finally(() => {
      if (!annullato) setGenerazioneCodiceLoading(false);
    });
    return () => {
      annullato = true;
    };
  }, [nuovoCespiteOpen, codiceCespiteAutomatico, formNuovoCespite.sede]);
  useEffect(() => {
  ...
  true)), [cespiti, dizionariCoreFmed, extraSedi, filtraDizionarioRimosso]);
  const sedi = useMemo(() => ["TUTTE", ...listaSedi], [listaSedi]);
  const listaReparti = useMemo(() => filtraDizionarioRimosso("reparto", listaPulitaDizionario(
    valoriDizionarioPrimari(dizionariCoreFmed, "reparti", [...REPARTI_STANDARD, ...extraReparti])
  )), [dizionariCoreFmed, extraReparti, filtraDizionarioRimosso]);
  const listaBranche = useMemo(() => filtraDizionarioRimosso("branca_medica", listaPulitaDizionario([
  ...valoriDizionarioPrimari(dizionariCoreFmed, "branche_mediche", [...BRANCHE_MEDICHE_STANDARD, ...extraBranche]),
  ...cespiti.map((c) => c.branca_medica || c.branca)]
  )), [cespiti, dizionariCoreFmed, extraBranche, filtraDizionarioRimosso]);
    ...
    return <div className="fmed-style-export-filter-box">
        <div className="fmed-literal-d1380c0341">
          ...
          <div className="fmed-literal-cc9c605f61">
            ...
            <button type="button" onClick={() => setter([...listaBranche])} className="fmed-style-mini-action-btn">Tutte</button>
            <button type="button" onClick={() => setter([])} className="fmed-style-mini-action-btn">Nessuna</button>
          </div>
        </div>
    ...
    return <div className="fmed-style-export-filter-box">
        <div className="fmed-literal-d1380c0341">
          ...
          <div className="fmed-literal-cc9c605f61">
            ...
            <button type="button" onClick={() => setExportSediInterventi([...listaSediInterventi])} className="fmed-style-mini-action-btn">Tutte</button>
            <button type="button" onClick={() => setExportSediInterventi([])} className="fmed-style-mini-action-btn">Nessuna</button>
          </div>
        </div>
  ...
  const listeDizionariAsset = useMemo(() => ({
    listaTipologie: filtraDizionarioRimosso("tipologia", listaPulitaDizionario([
    ...valoriDizionarioPrimari(dizionariCoreFmed, "tipologie_asset", extraTipologie),
    ...cespiti.map((c) => c.tipologia)]
    )),
    listaCostruttori: filtraDizionarioRimosso("costruttore", listaPulitaDizionario([
    ...valoriDizionarioPrimari(dizionariCoreFmed, "costruttori", extraCostruttori),
    ...cespiti.map((c) => c.costruttore)]
    )),
    listaModelli: filtraDizionarioRimosso("modello", listaPulitaDizionario([
    ...valoriDizionarioPrimari(dizionariCoreFmed, "modelli", extraModelli),
    ...cespiti.map((c) => c.modello)]
    )),
    listaFornitori: filtraDizionarioRimosso("fornitore", listaPulitaDizionario([
    ...valoriDizionarioPrimari(dizionariCoreFmed, "fornitori", extraFornitori),
    ...cespiti.map((c) => c.fornitore)]
    )),
    listaSocieta: filtraDizionarioRimosso("societa", listaPulitaDizionario([
    ...valoriDizionarioPrimari(dizionariCoreFmed, "societa", extraSocieta),
    ...cespiti.map((c) => c.societa)]
    )),
    listaLocazioni: filtraDizionarioRimosso("locazione", listaPulitaDizionario([
    ...valoriDizionarioPrimari(dizionariCoreFmed, "locazioni", extraLocazioni),
    ...cespiti.map((c) => getLocazioneFmed(c))]
    ))
  }), [cespiti, dizionariCoreFmed, extraTipologie, extraCostruttori, extraModelli, extraFornitori, extraSocieta, extraLocazioni, filtraDizionarioRimosso]);
  const {
    listaTipologie,
    listaCostruttori,
    listaModelli,
  ...
  const listeDizionariOperativi = useMemo(() => {
    ...
    return {
      listaCategorie: filtraDizionarioRimosso("categoria", listaPulitaDizionario([
      ...valoriDizionarioPrimari(dizionariCoreFmed, "categorie_asset", extraCategorie),
      ...cespiti.map((c) => normalizeCategoria(c.categoria))]
      )),
      listaPossesso: filtraDizionarioRimosso("possesso", listaPulitaDizionario([
      ...valoriDizionarioPrimari(dizionariCoreFmed, "possesso", [...POSSESSO_STANDARD, ...extraPossesso]),
      ...cespiti.map((c) => c.possesso)]
      )),
      listaStatiAsset: filtraDizionarioRimosso("stato_asset", listaPulitaDizionario(
        valoriDizionarioPrimari(dizionariCoreFmed, "stati_asset", [...STATI_ASSET_STANDARD, ...extraStatiAsset])
      )),
      ...
      ...valoriDizionarioPrimari(dizionariCoreFmed, "esiti_intervento", [...ESITI_STANDARD, ...extraEsiti]),
      ...interventi.map((i) => i.esito)]
      )),
      listaPriorita: filtraDizionarioRimosso("priorita", listaPulitaDizionario(
        valoriDizionarioPrimari(dizionariCoreFmed, "priorita", [...PRIORITA_STANDARD, ...extraPriorita])
      )),
  ...
  function valoreCostoIntervento(intervento = {}) {
    const costoCanonico = normalizzaNumeroCosto(intervento?.costo);
    const extraCanonico = normalizzaNumeroCosto(intervento?.importo_extra);
    if (costoCanonico || extraCanonico) return costoCanonico + extraCanonico;
    const campiCosto = [intervento?.Costo, intervento?.COSTO, intervento?.Importo, intervento?.IMPORTO, intervento?.importo, intervento?.totale, intervento?.Totale, intervento?.TOTALE, intervento?.prezzo, intervento?.Prezzo, intervento?.PREZZO, intervento?.spesa, intervento?.Spesa, intervento?.SPESA];
  ...
  function esportaInterventiFiltratiPdf() {
    const righe = interventiFiltrati.map((i) => ({
      ...
      costo: formatCurrency(importoIntervento(i))
    }));
    if (righe.length === 0) {
    ...
    if (!finestra) {
      alert("Il browser ha bloccato la nuova scheda. Consenti l’apertura di schede per esportare il PDF.");
      return;
    }
  ...
  const scadenzeFiltrate = useMemo(() => {
    // REV0: il backend è l'unica fonte dello scadenziario operativo globale.
  ...
  async function cessaScadenzeSelezionate(statoOverride = "", righeOverride = null) {
    ...
    const motivo = String(motivoChiusuraScadenze || motivoPredefinito).trim();
    if (motivo.length < 5) {
      setMessaggioCessazioneScadenze("Inserisci un motivo di almeno 5 caratteri prima di chiudere le attività.");
      return;
    }
    ...
    try {
      ...
      // REV0: aggiornamento locale immediato. Appena il backend conferma la
      ...
      void caricaScadenzeOnDemand({ force: true }).catch((refreshError) => {
        console.warn("[FMED REV0] Riconciliazione scadenze rinviata:", refreshError);
      });
    } catch (error) {
  ...
  async function generaCodiceCespiteDaBackend(sedeSelezionata) {
    ...
    try {
      setGenerazioneCodiceLoading(true);
      const data = await chiamataApiAutenticataFmed(`/cespite/prossimo-codice?sede=${encodeURIComponent(sedePulita)}`, {
        method: "GET"
      });
      const codiceGenerato = data?.prossimo_codice;
      if (!codiceGenerato) throw new Error("Il backend non ha restituito il codice inventario");
      return String(codiceGenerato || "").trim();
    } catch (err) {
      console.error("Errore generazione codice backend:", err);
...
` + `Interventi aggiornati: ${interventiDaAggiornare.length}`);
  ...
  function apriNuovoIntervento(cespite = null) {
    setCespiteRitornoOperazione(cespite || null);
    setProcessoNuovoInterventoGuidato(false);
    setProcessoGuidatoEsecuzioneId(null);
    resetFormNuovoIntervento(cespite);
    if (cespite) setCespiteSelezionato(null);
    setNuovoInterventoOpen(true);
  }

  async function aggiornaProcessoNuovoIntervento(payload) {
    if (!processoGuidatoEsecuzioneId) return false;
    try {
      await chiamataApiAutenticataFmed(`/core/processi/esecuzioni/${processoGuidatoEsecuzioneId}`, {
        method: "PATCH",
        body: JSON.stringify(payload)
      });
      return true;
  ...
  async function selezionaCespiteNuovoIntervento(cespite) {
    ...
    try {
      ...
      if (id) {
        await chiamataApiAutenticataFmed(`/core/processi/esecuzioni/${id}`, {
          method: "PATCH",
          body: JSON.stringify({
  ...
  async function chiudiNuovoIntervento({ annullaProcesso = true } = {}) {
    if (annullaProcesso && processoNuovoInterventoGuidato && processoGuidatoEsecuzioneId) {
      const annullato = await aggiornaProcessoNuovoIntervento({
        stato: "ANNULLATO",
        passo_corrente: "ANNULLATO_DALL_UTENTE",
        errori: []
      });
      if (!annullato) {
        alert("Il processo non è stato annullato dal backend. La pagina resta aperta.");
        return false;
      }
    }
    ...
    setCespiteRitornoOperazione(null);
    if (ritornoCespite) await apriSchedaCespite(ritornoCespite);
    return true;
  }
  ...
  function apriModificaIntervento(intervento) {
    setCespiteRitornoOperazione(cespiteSelezionato || null);
    setCespiteSelezionato(null);
    setModificaCespite(false);
    setModificaLinkDoc(false);
    setStoricoCespiteOpen(false);
  ...
  async function salvaModificaIntervento() {
    ...
    try {
      ...
      const ritornoCespite = cespiteRitornoOperazione; setModificaInterventoOpen(false); setInterventoDaModificare(null); setFormModificaIntervento({}); setCespiteRitornoOperazione(null); if (ritornoCespite) await apriSchedaCespite(ritornoCespite); alert("Intervento modificato correttamente");
    } catch (err) {
  ...
  function exportBudgetCriticitaFmed(formato = "excel") {
    ...
    interventi.forEach((i) => {
      ...
      if (isStraordinaria || isFermo) {
        gruppi[codice].costoTotale += importoIntervento(i);
      }
    });
  ...
  async function ricaricaDatiInterventi(codice = null) {
    try {
      ...
      if (codicePulito) {
        ...
        const [storicoRes, analisiRes] = await Promise.all([
        fetch(`${API_BASE_URL}/interventi-cespite/${codiceUrl}`, { headers: fmedAuthHeaders() }),
        fetch(`${API_BASE_URL}/analisi/${codiceUrl}`, { headers: fmedAuthHeaders() })]
        );
        if (storicoRes.ok) {
  ...
  function nuovoFormInfrastruttura(base = {}) {
    const sede = normalizzaSedeDisplay(base.sede || "");
    return {
      codice: base.codice || "",
      sede,
      centro_costo: normalizzaCentroCostoFmed(base.centro_costo, sede) || "",
      categoria: base.categoria || infraCategorieStandard[0] || "",
      descrizione: base.descrizione || "",
      ditta: base.ditta || infraDitteOptions[0] || "",
      periodicita: base.periodicita || "",
      ...
      stato: base.stato || infraStatiStandard[0] || "",
  ...
  const infraCategorieStandard = pulisciLista([
  ...etichetteDaDizionario(dizionariCoreFmed, "categorie_infrastrutture", []),
  ...infrastruttureConStato.map((r) => r.categoria)]
  );
  const infraPeriodicitaStandard = pulisciLista([
  ...codiciDaDizionario(dizionariCoreFmed, "periodicita", PERIODICITA_STANDARD.map((item) => item.codice)),
  ...infrastruttureConStato.map((r) => r.periodicita)]
  );
  const infraStatiStandard = pulisciLista([
  ...codiciDaDizionario(dizionariCoreFmed, "stati_infrastruttura", []),
  ...infrastruttureConStato.map((r) => r.stato)]
  );
  const infraDitteOptions = pulisciLista([
  ...etichetteDaDizionario(dizionariCoreFmed, "ditte_esecutrici", []),
  ...infrastruttureConStato.map((r) => normalizzaSocietaDitta(r.ditta))]
  );
  const infraSocietaOptions = pulisciLista([
  ...etichetteDaDizionario(dizionariCoreFmed, "societa", []),
  ...infrastruttureConStato.map((r) => normalizzaSocietaDitta(r.societa))]
  );
  const infraCentroCostoOptions = pulisciLista([
  ...etichetteDaDizionario(dizionariCoreFmed, "centri_costo", []),
  ...infrastruttureConStato.map((r) => normalizzaCentroCostoFmed(r.centro_costo, r.sede))]
  );
  ...
  function apriMenuAttivitaInfrastruttura(record, tipo = "ordinarie") {
    const destinazione = ["ordinarie", "straordinarie"].includes(tipo) ? tipo : "ordinarie";
    apriDocumentoInfrastruttura(record, destinazione);
  }
  function apriMenuDocumentazioneInfrastruttura(record, tipo = "generale") {
    const tipiAmmessi = ["generale", "fotografie", "schemi", "certificazioni", "contratti", "costi", "manuali"];
    apriDocumentoInfrastruttura(record, tipiAmmessi.includes(tipo) ? tipo : "generale");
  }
  function apriMenuAllegaInfrastruttura(record, tipo = "straordinarie") {
    const tipiAmmessi = ["ordinarie", "straordinarie", "fotografie", "schemi", "certificazioni", "contratti", "costi", "manuali", "generale"];
    apriDocumentoInfrastruttura(record, tipiAmmessi.includes(tipo) ? tipo : "straordinarie");
  }
  ...
  const paginaOperativaSecondariaAperta = Boolean(
    alertMailOpen ||
    nuovoInterventoOpen ||
    modificaInterventoOpen ||
    nuovoCespiteOpen ||
    gestioneDizionariOpen ||
    cespiteSelezionato
  );

  return <div data-fmed-build={MRDB_APP_VERSION} className="fmed-app-root fmed-light-mode fmed-shell-premium fmed-visual-clean fmed-style-app fmed-style-theme-light-vars" style={{
    ...
    ...{}
      ...
      {<aside className={[`fmed-side-rail ${sidebarCompattata ? "is-collapsed" : "is-expanded"}`, "fmed-style-sidebar", sidebarCompattata && "fmed-style-sidebar-collapsed"].filter(Boolean).join(" ")}>
        ...
        <div className={`fmed-sidebar-controls ${sidebarCompattata ? "is-collapsed fmed-style-sidebar-collapse-top-centered" : "fmed-style-sidebar-collapse-top"}`}>
          <button
          type="button"
          className="fmed-sidebar-control-btn fmed-sidebar-collapse-btn fmed-style-sidebar-collapse-btn"
        ...
        <div className="fmed-style-sidebar-divider" />

        <nav className="fmed-sidebar-navigation fmed-style-sidebar-nav" aria-label="Funzioni principali FMED">
          {FMED_MENU_GROUPS.map((group) => {
          const visibleItems = group.items.filter((item) => menuItemsFmed.includes(item));
          if (visibleItems.length === 0) return null;
          return <section className="fmed-sidebar-menu-group" key={group.label} aria-label={group.label}>
              {!sidebarCompattata && <div className="fmed-sidebar-group-label">{group.label}</div>}
              <div className="fmed-sidebar-group-items">
                ...
                return <button key={item} className={[`fmed-side-menu-btn ${active ? "is-active" : ""}`, "fmed-style-menu-btn", active && "fmed-style-menu-btn-active"].filter(Boolean).join(" ")} title={help} data-help={help} data-module={item} aria-current={active ? "page" : undefined} aria-label={`${fmedMenuLabel(item)}. ${help}`} onClick={() => {
                  ...
                  setPagina(item);
                    ...
                    <span className="fmed-side-menu-icon" aria-hidden="true">{renderFmedMenuIcon(item)}</span>
                    {!sidebarCompattata && <span className="fmed-side-menu-label">{fmedMenuLabel(item)}</span>}
                  </button>;
              })}
              </div>
            </section>;
        })}
          <section className="fmed-sidebar-menu-group fmed-sidebar-external-group" aria-label="Collegamenti">
            {!sidebarCompattata && <div className="fmed-sidebar-group-label">Collegamenti</div>}
            <div className="fmed-sidebar-group-items">
              <a
              ...
              target="_blank"
              rel="noreferrer"
              title="Apri Marilab Mover in una nuova scheda"
              aria-label="Apri Marilab Mover in una nuova scheda">
              
                <span className="fmed-side-menu-icon" aria-hidden="true">
                  <svg viewBox="0 0 24 24">
                    <path d="M3.5 7.5h10v8h-10zM13.5 10h3.2l3.8 3.4v2.1h-7zM7 18a1.8 1.8 0 1 0 0-3.6A1.8 1.8 0 0 0 7 18Zm10.5 0a1.8 1.8 0 1 0 0-3.6 1.8 1.8 0 0 0 0 3.6Z" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </span>
        ...
        <div className={`fmed-sidebar-footer ${sidebarCompattata ? "is-collapsed" : ""}`}>
          {sessioneFmed && <div className={`fmed-sidebar-account ${sidebarCompattata ? "is-collapsed" : ""}`}>
            <div className="fmed-sidebar-account-avatar">
              {String(utenteCorrenteFmed || sessioneFmed.role || "F").slice(0, 1).toUpperCase()}
            </div>
            {!sidebarCompattata && <div className="fmed-sidebar-account-copy">
              <strong>{utenteCorrenteFmed}</strong>
              <span>{permessiRuoloFmed.label || sessioneFmed.label || sessioneFmed.role}</span>
            </div>}
            {onLogoutFmed && <button type="button" className="fmed-sidebar-logout" onClick={() => {
            rimuoviSessioneFmedSalvata();
            if (onLogoutFmed) onLogoutFmed();else window.location.reload();
          }} title="Esci da FMED" aria-label="Esci da FMED">
              <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 5H6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h4M14 8l4 4-4 4M9 12h9" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </button>}
          </div>}
        </div>
      ...
      <main
      className="fmed-main-content fmed-style-main"
      data-fmed-page={pagina}
      data-fmed-workspace-open={paginaOperativaSecondariaAperta ? "true" : "false"}
      style={{
        ...
        {paginaScadenzeAttiva && scadenzeElencoAperto && <section
        ...
        aria-label="Gestione scadenze obsolete">
          ...
          <div className="fmed-literal-f318d11f4b">
            ...
            "Motivo (predefinito: non applicabile)"} />
        ...
        {pagina === "Processi" && permessiRuoloFmed.canManageProcesses && <Suspense fallback={<div className="fmed-style-card">Caricamento Processi…</div>}>
            <ProcessiPage
          apiBaseUrl={API_BASE_URL}
...
{paginaExportAttiva && permessiRuoloFmed.canExport && <Suspense fallback={<div className="fmed-lazy-loading">Caricamento modulo…</div>}><ExportPage {...{
        ...
        {pagina === "Nuovo Asset" && nuovoAssetWizardOpen && <Suspense fallback={<div className="fmed-style-card">Caricamento processo Nuovo Asset…</div>}>
            <NewAssetWizard apiBaseUrl={API_BASE_URL} dizionari={dizionariCoreFmed} user={utenteCorrenteFmed || sessioneFmed?.email || "FMED"} executionId={processoGuidatoEsecuzioneId} initialData={nuovoAssetWizardInitialData} generateInventoryCode={generaCodiceCespiteDaBackend} onOpenOcr={(bozzaWizard) => {
          resetFormNuovoCespite();
          setFormNuovoCespite((prev) => ({
            ...prev,
            ...(bozzaWizard || {})
        ...
        {(pagina === "Gestione Utenti" || pagina === "Dizionari") && permessiRuoloFmed.canManageUsers && <Suspense fallback={<div className="fmed-style-card">Caricamento Impostazioni…</div>}>
            <ImpostazioniPage
          apiBaseUrl={API_BASE_URL}
          canManage={permessiRuoloFmed.canManageUsers}
    ...
    <div className="fmed-workspace-surface fmed-intervention-workspace-surface">
      ...
      {(!processoNuovoInterventoGuidato || formNuovoIntervento.codice_strumento) && <div className="fmed-new-intervention-form fmed-style-edit-panel">
        ...
        <div style={{
          ...
          <CanonicalSelect
                ...
                onChange={(value) => setFormNuovoIntervento((prev) => aggiornaFormInterventoConScadenza(prev, "periodicita", value))} className="fmed-style-edit-field" />
              

          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Data ultimo intervento</label>
            <input type="date" value={formNuovoIntervento.data_ultimo_intervento || ""} onChange={(e) => setFormNuovoIntervento((prev) => aggiornaFormInterventoConScadenza(prev, "data_ultimo_intervento", e.target.value))} className="fmed-style-edit-input" />
          </div>

          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Data prossimo intervento</label>
            <input type="date" value={formNuovoIntervento.data_prossimo_intervento || ""} onChange={(e) => setFormNuovoIntervento({
                  ...formNuovoIntervento,
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Costo (€)</label>
            <input inputMode="decimal" value={formNuovoIntervento.costo || ""} onChange={(e) => setFormNuovoIntervento({
                  ...formNuovoIntervento,
                  costo: e.target.value
                })} placeholder="€ 250,00" className="fmed-style-edit-input" />
          </div>

          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Importo extra (€)</label>
            <input inputMode="decimal" value={formNuovoIntervento.importo_extra || ""} onChange={(e) => setFormNuovoIntervento({
                  ...formNuovoIntervento,
                  importo_extra: e.target.value
                })} placeholder="€ 80,00" className="fmed-style-edit-input" />
          </div>
          ...
          <CanonicalSelect
                ...
                onChange={(value) => setFormNuovoIntervento((prev) => ({ ...prev, esito: value }))} className="fmed-style-edit-field" />
              

          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Link documento / Job report</label>
            <input value={formNuovoIntervento.link_documento || ""} onChange={(e) => setFormNuovoIntervento({
                  ...formNuovoIntervento,
                  link_documento: e.target.value
                })} placeholder="https://..." className="fmed-style-edit-input" />
          </div>
          ...
          <div style={{
            ...
            <label className="fmed-style-edit-label">Descrizione attività</label>
            <textarea value={formNuovoIntervento.descrizione_attivita || ""} onChange={(e) => setFormNuovoIntervento({
                  ...formNuovoIntervento,
                  descrizione_attivita: e.target.value
                })} placeholder="Descrivi intervento, ricambi, anomalie, note tecniche..." className="fmed-style-edit-textarea" />
          </div>
        </div>
        ...
        <div className="fmed-workspace-actions fmed-style-edit-actions">
          <button type="button" onClick={() => chiudiNuovoIntervento({ annullaProcesso: true })} className="fmed-style-cancel-btn">
            Annulla
          </button>

          <button type="button" onClick={salvaNuovoIntervento} disabled={!String(formNuovoIntervento.codice_strumento || "").trim()} title={!String(formNuovoIntervento.codice_strumento || "").trim() ? "Seleziona prima un cespite" : "Salva il nuovo intervento"} className="fmed-style-save-btn">
        ...
        {modificaInterventoOpen && <section className="fmed-workspace-page fmed-intervention-workspace fmed-edit-intervention-page" aria-labelledby="fmed-edit-intervention-title">
    <div className="fmed-workspace-surface fmed-intervention-workspace-surface">
      <button className="fmed-workspace-back fmed-style-close-btn" onClick={() => { const ritorno = cespiteRitornoOperazione; setModificaInterventoOpen(false); setInterventoDaModificare(null); setFormModificaIntervento({}); setCespiteRitornoOperazione(null); if (ritorno) apriSchedaCespite(ritorno); }}>
        <FmedIcon name="close" /> {cespiteRitornoOperazione ? "Torna indietro" : "Torna indietro"}
      </button>

      <h2 id="fmed-edit-intervention-title" className="fmed-style-workspace-title">Modifica intervento</h2>
      <p className="fmed-workspace-description">Aggiorna i dati dell’intervento nella pagina completa mantenendo invariati filtri e posizione dell’elenco.</p>

      <div className="fmed-style-edit-panel">
        <div style={{
          ...
          <CanonicalSelect
                ...
                onChange={(value) => setFormModificaIntervento((prev) => aggiornaFormInterventoConScadenza(prev, "periodicita", value))} className="fmed-style-edit-field" />
              

          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Ultimo intervento</label>
            <input type="date" value={formModificaIntervento.data_ultimo_intervento || ""} onChange={(e) => setFormModificaIntervento((prev) => aggiornaFormInterventoConScadenza(prev, "data_ultimo_intervento", e.target.value))} className="fmed-style-edit-input" />
          </div>

          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Prossimo intervento</label>
            <input type="date" value={formModificaIntervento.data_prossimo_intervento || ""} onChange={(e) => setFormModificaIntervento({
                  ...formModificaIntervento,
                  data_prossimo_intervento: e.target.value
                })} className="fmed-style-edit-input" />
          </div>
          ...
          <CanonicalSelect
                ...
                onChange={(value) => setFormModificaIntervento((prev) => ({ ...prev, esito: value }))} className="fmed-style-edit-field" />
              

          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Costo (€)</label>
            <input inputMode="decimal" value={formModificaIntervento.costo || ""} onChange={(e) => setFormModificaIntervento({
                  ...formModificaIntervento,
                  costo: e.target.value
                })} placeholder="€ 250,00" className="fmed-style-edit-input" />
          </div>
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Importo extra (€)</label>
            <input inputMode="decimal" value={formModificaIntervento.importo_extra || ""} onChange={(e) => setFormModificaIntervento({
                  ...formModificaIntervento,
                  importo_extra: e.target.value
                })} placeholder="€ 80,00" className="fmed-style-edit-input" />
          </div>
          ...
          <div style={{
            ...
            <label className="fmed-style-edit-label">Link documento / Job report</label>
            <input value={formModificaIntervento.link_documento || ""} onChange={(e) => setFormModificaIntervento({
                  ...formModificaIntervento,
                  link_documento: e.target.value
                })} placeholder="https://..." className="fmed-style-edit-input" />
          </div>
          ...
          <div style={{
            ...
            <label className="fmed-style-edit-label">Descrizione attività</label>
            <textarea value={formModificaIntervento.descrizione_attivita || ""} onChange={(e) => setFormModificaIntervento({
                  ...formModificaIntervento,
                  descrizione_attivita: e.target.value
                })} placeholder="Correggi descrizione, note tecniche, refusi, ricambi..." className="fmed-style-edit-textarea" />
          </div>
        </div>

        <div className="fmed-workspace-actions fmed-style-edit-actions">
          <button onClick={() => { const ritorno = cespiteRitornoOperazione; setModificaInterventoOpen(false); setInterventoDaModificare(null); setFormModificaIntervento({}); setCespiteRitornoOperazione(null); if (ritorno) apriSchedaCespite(ritorno); }} className="fmed-style-cancel-btn">
            Annulla
          </button>

          <button onClick={salvaModificaIntervento} className="fmed-style-save-btn">
             Salva modifiche
          </button>
        </div>
      </div>
        ...
        {nuovoCespiteOpen && <section className="fmed-workspace-page fmed-asset-capture-workspace" aria-labelledby="fmed-asset-capture-title">
    <div className="fmed-workspace-surface fmed-asset-capture-surface">
      <button className="fmed-workspace-back fmed-style-close-btn" onClick={() => {
            setNuovoCespiteOpen(false);
            setNuovoAssetWizardInitialData({});
            setProcessoGuidatoEsecuzioneId(null);
            resetFormNuovoCespite();
          }}>
        <FmedIcon name="close" /> Torna al wizard
      </button>

      <h2 id="fmed-asset-capture-title" className="fmed-style-workspace-title">
        {processoGuidatoEsecuzioneId ? " Acquisizione OCR per wizard FMED" : " Aggiungi nuovo cespite"}
      </h2>
      ...
      <div className="fmed-literal-bec04a7b4e">
        ...
        <div className="fmed-style-ocr-card-row">
          ...
          <label style={{
            ...
            <input id="fmed-ocr-primary-input" type="file" accept="image/*" disabled={ocrTarghettaLoading}
        ...
        {ocrTarghettaEsito && <div style={{
            ...
            {ocrTarghettaEsito.status === "error" ? " " : " "}
      ...
      <div className="fmed-literal-63b4bb00c8">
        ...
        <div className="fmed-style-ocr-card-row">
          ...
          <label style={{
            ...
            <input id="fmed-ocr-accessory-input" type="file" accept="image/*" disabled={ocrAccessorioLoading}
        ...
        {ocrAccessorioEsito && <div style={{
            ...
            {ocrAccessorioEsito.status === "error" ? " " : " "}
      ...
      <div className="fmed-style-edit-panel">
        <div style={{
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Codice inventario / strumento *</label>
            <input style={{
            ...
            <div className="fmed-literal-9ecfcc6921">
              ...
              <button type="button" style={{
                ...
                {generazioneCodiceLoading ? "⏳ Generazione..." : " Genera automatico"}
              </button>
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Matricola</label>
            <input value={formNuovoCespite.matricola || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
                  matricola: e.target.value
                })} className="fmed-style-edit-input" />
          </div>
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Anno</label>
            <input value={formNuovoCespite.anno_di_fabbricazione || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Marcatura CE</label>
            <input value={formNuovoCespite.marcatura_ce || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Alimentazione</label>
            <input value={formNuovoCespite.alimentazione || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Potenza</label>
            <input value={formNuovoCespite.potenza || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Frequenza</label>
            <input value={formNuovoCespite.frequenza || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Lotto</label>
            <input value={formNuovoCespite.lotto || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Prezzo</label>
            <input value={formNuovoCespite.prezzo || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
          ...
          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Data collaudo</label>
            <input value={formNuovoCespite.data_di_collaudo || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
                  data_di_collaudo: e.target.value
                })} placeholder="gg/mm/aaaa" className="fmed-style-edit-input" />
          </div>

          <div className="fmed-style-edit-field">
            <label className="fmed-style-edit-label">Data messa in uso</label>
            <input value={formNuovoCespite.data_messa_in_uso || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
          ...
          <div style={{
            ...
            <label className="fmed-style-edit-label">Accessori / Sistema primario</label>
            <textarea value={formNuovoCespite.accessori_sistema_primario || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
                  accessori_sistema_primario: e.target.value
                })} className="fmed-style-edit-textarea" />
          </div>
          ...
          <div style={{
            ...
            <label className="fmed-style-edit-label">Foto cespite / documentazione</label>
            <div style={{
            ...
            <div className="fmed-literal-0c46007ef8">
              ...
              {CATEGORIE_FOTO_CESPITE.map((foto) => <input key={foto.campo} value={formNuovoCespite[foto.campo] || ""} placeholder={`${foto.label} - link SharePoint / URL`} onChange={(e) => setFormNuovoCespite({
                    ...formNuovoCespite,
          ...
          <div style={{
            ...
            <label className="fmed-style-edit-label">Note</label>
            <textarea value={formNuovoCespite.note || ""} onChange={(e) => setFormNuovoCespite({
                  ...formNuovoCespite,
                  note: e.target.value
                })} className="fmed-style-edit-textarea" />
          </div>
        ...
        <div className="fmed-workspace-actions fmed-style-edit-actions">
          <button onClick={() => {
                ...
                setProcessoGuidatoEsecuzioneId(null);
                resetFormNuovoCespite();
              }} className="fmed-style-cancel-btn">
            Annulla
          </button>

          {processoGuidatoEsecuzioneId ? <button onClick={() => {
                setNuovoAssetWizardInitialData({
                  ...formNuovoCespite
                });
                setNuovoCespiteOpen(false);
                setNuovoAssetWizardOpen(true);
                setPagina("Nuovo Asset");
              }} className="fmed-style-save-btn">
              Continua nel wizard FMED →
            </button> : <button onClick={creaNuovoCespite} className="fmed-style-save-btn">
               Crea cespite
            </button>}
        </div>
      </div>
...
{gestioneDizionariOpen && <section className="fmed-workspace-page fmed-dictionary-workspace" aria-labelledby="fmed-dictionary-workspace-title">
    <div className="fmed-workspace-surface fmed-dictionary-workspace-surface">
      <button className="fmed-workspace-back fmed-style-close-btn" onClick={() => {
      ...
      <h2 id="fmed-dictionary-workspace-title" className="fmed-style-workspace-title">Gestione dizionari Asset</h2>
      <p style={{
      ...
      <div className="fmed-style-dictionary-grid">
        ...
        {cespiteSelezionato && <section className={["fmed-workspace-page", "fmed-asset-detail-workspace", analisiPredittivaAperta && "is-predictive-page"].filter(Boolean).join(" ")} aria-label={`SCHEDA CESPITE ${cespiteSelezionato.codicestrumento || ""}`}>
    <div className="fmed-workspace-surface fmed-asset-detail-page">
      ...
      {analisiCespite && <div className="fmed-style-asset-kpi-ribbon">
          <div className="fmed-style-asset-kpi-item">
            <span
              className="fmed-style-asset-kpi-icon-green"
              style={{
                background: coloreCriticita(analisiCespite.criticita).soft,
                border: coloreCriticita(analisiCespite.criticita).border
              }}
            />
            <div>
              <div className="fmed-style-asset-kpi-label">Criticità</div>
              <div
                className="fmed-style-asset-kpi-value"
                style={{ color: coloreCriticita(analisiCespite.criticita).color }}
              >
          ...
          <div className="fmed-style-asset-kpi-item">
            <span
              className="fmed-style-asset-kpi-icon-green"
              style={{
                background: coloreCriticita(analisiCespite.rischio).soft,
                border: coloreCriticita(analisiCespite.rischio).border
              }}
            />
            <div>
              <div className="fmed-style-asset-kpi-label">Rischio</div>
              <div
                className="fmed-style-asset-kpi-value"
                style={{ color: coloreCriticita(analisiCespite.rischio).color }}
              >
          ...
          <div className="fmed-style-asset-kpi-item">
            <span className="fmed-style-asset-kpi-icon-blue">⏱</span>
            <div>
              <div className="fmed-style-asset-kpi-label">Punteggio</div>
              <div className="fmed-style-asset-kpi-value">
                {analisiCespite.punteggio ?? "-"}/100
              </div>
            </div>
          </div>

          <div className="fmed-style-asset-kpi-item">
            <span className="fmed-style-asset-kpi-icon-purple"></span>
            <div>
              <div className="fmed-style-asset-kpi-label">Interventi</div>
              <div className="fmed-style-asset-kpi-value">
                {analisiCespite.interventi ?? interventiCespite.length}
              </div>
            </div>
          </div>

          <div className="fmed-style-asset-kpi-item">
            <span className="fmed-style-asset-kpi-icon-orange"></span>
            <div>
              <div className="fmed-style-asset-kpi-label">Prossimo intervento</div>
              <div className="fmed-style-asset-kpi-value-small">
                {getProssimoInterventoValido(interventiCespite)?.data_prossimo_intervento
                  ? formattaData(getProssimoInterventoValido(interventiCespite)?.data_prossimo_intervento)
                  : "-"}
              </div>
            </div>
          </div>
      ...
      <div className="fmed-style-asset-actions-panel">
        ...
        <div className="fmed-style-asset-actions-navigation">
          <button type="button" className="fmed-style-asset-action-btn fmed-style-asset-action-btn-predictive" onClick={() => setAnalisiPredittivaAperta(true)} disabled={!analisiCespite}>
            <span className="fmed-style-asset-action-icon"><FmedIcon name="chart" /></span>
            <span>Analisi predittiva</span>
          </button>
          <button type="button" className="fmed-style-asset-action-btn fmed-style-asset-action-btn-back" onClick={() => { if (analisiPredittivaAperta) { setAnalisiPredittivaAperta(false); return; } setCespiteSelezionato(null); }}>
            <span className="fmed-style-asset-action-icon"><FmedIcon name="close" /></span>
            <span>{analisiPredittivaAperta ? "Torna indietro" : "Torna indietro"}</span>
          </button>
                  <button type="button" className="fmed-style-asset-action-btn fmed-style-asset-action-btn-scan" onClick={() => document.getElementById("fmed-ocr-primary-input")?.click()} disabled={ocrTarghettaLoading}>
            <span className="fmed-style-asset-action-icon"><FmedIcon name="qr" /></span>
            <span>{ocrTarghettaLoading ? "Lettura in corso..." : "Scansiona sistema primario"}</span>
          </button>
          <button type="button" className="fmed-style-asset-action-btn fmed-style-asset-action-btn-accessory" onClick={() => document.getElementById("fmed-ocr-accessory-input")?.click()} disabled={ocrAccessorioLoading}>
            <span className="fmed-style-asset-action-icon"><FmedIcon name="plus" /></span>
            <span>{ocrAccessorioLoading ? "Lettura in corso..." : "Aggiungi accessorio"}</span>
          </button>
        </div>
        <div className="fmed-style-asset-actions-grouped">
          ...
          <div className="fmed-style-asset-actions-group-block">
            ...
            <div className="fmed-style-asset-actions-group-grid">
              ...
              <button onClick={() => setStoricoCespiteOpen(true)} className={["fmed-style-asset-action-btn", storicoCespiteOpen && "fmed-style-asset-action-btn-edit"].filter(Boolean).join(" ")}>
          ...
          <div className="fmed-style-asset-actions-group-block">
            ...
            <div className="fmed-style-asset-actions-group-grid">
              <button onClick={() => setModificaCespite(true)} className="fmed-style-asset-action-btn fmed-style-asset-action-btn-edit">


                    
                <span className="fmed-style-asset-action-icon"><FmedIcon name="edit" /></span>
                <span>Modifica cespite</span>
              </button>
...
{modificaCespite && <div className="fmed-style-edit-panel fmed-asset-edit-page-panel">
    <button type="button" className="fmed-workspace-back fmed-style-close-btn fmed-asset-edit-back" onClick={() => { setFormCespite(cespiteSelezionato); setModificaCespite(false); }}>
      <FmedIcon name="close" /> Torna indietro
    </button>
    ...
    <div className="fmed-literal-ad27e2bddd">
      ...
      {(ocrTarghettaEsito || ocrAccessorioEsito) && <div className="fmed-literal-e844ea5516">
          ...
          {ocrTarghettaEsito?.note ? <div>{ocrTarghettaEsito.status === "error" ? "" : ""} {ocrTarghettaEsito.note}</div> : null}
          {ocrAccessorioEsito?.note ? <div>{ocrAccessorioEsito.status === "error" ? "" : ""} {ocrAccessorioEsito.note}</div> : null}
        </div>}
    </div>
    ...
    <div style={{
      ...
      <div className="fmed-style-edit-field">
        ...
        {String(formCespite.codicestrumento || "").trim() !== String(cespiteSelezionato?.codicestrumento || "").trim() && <div className="fmed-literal-fbf730b1bf">
             ...
             Attenzione: stai modificando il codice inventario del cespite. Salva solo se vuoi correggere realmente il codice storico.
          </div>}
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Matricola</label>
        <input value={formCespite.matricola || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  matricola: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Anno</label>
        <input value={formCespite.anno_di_fabbricazione || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  anno_di_fabbricazione: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Marcatura CE</label>
        <input value={formCespite.marcatura_ce || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  marcatura_ce: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Alimentazione</label>
        <input value={formCespite.alimentazione || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  alimentazione: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Potenza</label>
        <input value={formCespite.potenza || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  potenza: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Frequenza</label>
        <input value={formCespite.frequenza || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  frequenza: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Lotto</label>
        <input value={formCespite.lotto || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  lotto: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Prezzo</label>
        <input value={formCespite.prezzo || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  prezzo: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Data collaudo</label>
        <input value={formattaDataInput(formCespite.data_di_collaudo)} onChange={(e) => setFormCespite({
                  ...formCespite,
                  data_di_collaudo: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div className="fmed-style-edit-field">
        <label className="fmed-style-edit-label">Data messa in uso</label>
        <input value={formattaDataInput(formCespite.data_messa_in_uso)} onChange={(e) => setFormCespite({
                  ...formCespite,
                  data_messa_in_uso: e.target.value
                })} className="fmed-style-edit-input" />
      </div>
      ...
      <div style={{
        ...
        <label className="fmed-style-edit-label">Note</label>
        <textarea value={formCespite.note || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  note: e.target.value
                })} className="fmed-style-edit-textarea" />
      </div>
      ...
      <div style={{
        ...
        <label className="fmed-style-edit-label">Accessori / Sistema primario</label>
        <textarea value={formCespite.accessori_sistema_primario || ""} onChange={(e) => setFormCespite({
                  ...formCespite,
                  accessori_sistema_primario: e.target.value
                })} className="fmed-style-edit-textarea" />
      </div>
    ...
    <div className="fmed-workspace-actions fmed-style-edit-actions">
      ...
      <button onClick={salvaModificheCespite} className="fmed-style-save-btn">
         Salva modifiche
      </button>
    </div>
  </div>}
      ...
      {storicoCespiteOpen && <div className="fmed-style-asset-history-collapsible">
          <div className="fmed-style-asset-history-header-compact">
            ...
            {<div className="fmed-style-table-wrap-compact">
                <table className="fmed-style-table">
                  <thead>
                    <tr>
                      <th className="fmed-style-th">Data</th>
                      <th className="fmed-style-th">Attività</th>
                      <th className="fmed-style-th">Ditta</th>
                      <th className="fmed-style-th">Prossimo</th>
                      <th className="fmed-style-th">Documento</th>
                    </tr>
                  </thead>

                  <tbody>
                    {storicoCespiteFiltrato.map((i, index) => <tr key={i.id_intervento || i.id || index} className="fmed-style-tr">
                        <td className="fmed-style-td">{formattaData(i.data_ultimo_intervento)}</td>
                        <td className="fmed-style-td">{i.attivita || "-"}</td>
                        <td className="fmed-style-td">{i.ditta_esecutrice || i.ditta || "-"}</td>
                        <td className="fmed-style-td">{formattaData(i.data_prossimo_intervento)}</td>
                        <td className="fmed-style-td">
                          <div className="fmed-style-row-action-group">
                            <BottoneJobReport intervento={i} />
                            <button type="button" title="Modifica questo intervento" onClick={(e) => {
                          e.stopPropagation();
                          apriModificaIntervento(i);
                        }} className="fmed-style-action-btn-edit"> Modifica</button>
                            <button type="button" title="Elimina questo intervento" onClick={(e) => {
                          e.stopPropagation();
                          eliminaIntervento(i);
                        }} className="fmed-style-action-btn-delete"> Elimina</button>
                          </div>
                        </td>
                      </tr>)}
                    {!storicoCespiteFiltrato.length && <tr><td colSpan="5" className="fmed-style-td">Nessun intervento trovato con i filtri selezionati.</td></tr>}
                  </tbody>
                </table>
              </div>}
        </div>}
      ...
      <div className="fmed-asset-data-fullwidth">
      ...
      {analisiCespite && <div className="fmed-asset-predictive-horizontal">{(() => {
                ...
                return <div className="fmed-asset-predictive-analysis fmed-predictive-panel fmed-style-asset-panel fmed-style-predictive-panel" style={{


                  borderColor: stileCriticita.border.replace("1px solid ", "")
                }}>
            <div className="fmed-style-predictive-header">
              <div>
                <h3 style={{ marginBottom: 3 }} className="fmed-style-section-title">Analisi predittiva</h3>
                <div className="fmed-style-predictive-subtitle">Lettura immediata dello stato tecnico e del rischio operativo.</div>
              </div>
              <span style={{
                    ...
                    }} className="fmed-style-predictive-summary-pill">
                ● {analisiCespite.criticita || "Da valutare"}
              </span>
            </div>
            <div className="fmed-predictive-grid fmed-style-fmea-grid">
              <div className="fmed-predictive-card is-criticality fmed-style-fmea-box" style={{ background: stileCriticita.background, border: stileCriticita.border, boxShadow: `inset 4px 0 0 ${stileCriticita.accent}` }}>
                <span style={{ background: stileCriticita.soft, color: stileCriticita.color }} className="fmed-style-predictive-icon"></span>
                <span className="fmed-style-predictive-metric-label">Criticità</span>
                <strong style={{ color: stileCriticita.color, fontSize: "22px" }}>{analisiCespite.criticita || "-"}</strong>
                <small className="fmed-style-predictive-metric-hint">Condizione complessiva</small>
              </div>
              <div className="fmed-predictive-card is-risk fmed-style-fmea-box" style={{ background: stileRischio.background, border: stileRischio.border, boxShadow: `inset 4px 0 0 ${stileRischio.accent}` }}>
                <span style={{ background: stileRischio.soft, color: stileRischio.color }} className="fmed-style-predictive-icon"></span>
                <span className="fmed-style-predictive-metric-label">Rischio</span>
                <strong style={{ color: stileRischio.color, fontSize: "22px" }}>{analisiCespite.rischio || "-"}</strong>
                <small className="fmed-style-predictive-metric-hint">Probabilità di criticità</small>
              </div>
              <div className="fmed-predictive-card is-score fmed-style-fmea-box" style={{ background: stilePunteggio.soft, border: `1px solid ${stilePunteggio.border}`, boxShadow: `inset 4px 0 0 ${stilePunteggio.accent}` }}>
                <span style={{ background: "rgba(255,255,255,.72)", color: stilePunteggio.color }} className="fmed-style-predictive-icon"></span>
                <span className="fmed-style-predictive-metric-label">Punteggio</span>
                <strong style={{ fontSize: "22px", color: stilePunteggio.color }}>{analisiCespite.punteggio ?? "-"}/100</strong>
                <div aria-label={`Punteggio predittivo ${punteggio} su 100`} className="fmed-style-predictive-progress-track">
                  <span style={{ width: `${punteggio}%`, background: stilePunteggio.accent }} className="fmed-style-predictive-progress-fill" />
                </div>
              </div>
        ...
        <div className="fmed-asset-predictive-evaluations fmed-predictive-recommendations fmed-style-recommendation-panel">
          <h3 className="fmed-style-section-title">Valutazioni predittive</h3>
          <div style={{

                  ...{}
                }} className="fmed-style-recommendation-grid">
            <div className="fmed-style-recommendation-box fmed-style-recommendation-box-info">
              <div style={{ color: "#175CD3" }} className="fmed-style-recommendation-title">ℹ Raccomandazione</div>
              <p>{safeText(analisiCespite.raccomandazione)}</p>
            </div>
            <div className="fmed-style-recommendation-box fmed-style-recommendation-box-technical">
              <div style={{ color: "#6941C6" }} className="fmed-style-recommendation-title"> Motivazione tecnica</div>
              <p>{safeText(analisiCespite.motivazione)}</p>
            </div>
            <div className="fmed-style-recommendation-box fmed-style-recommendation-box-calculation">
              <div style={{ color: "#B54708" }} className="fmed-style-recommendation-title"> Criterio di calcolo</div>
              <p>
...
function DizionarioBox({
  ...
  return <div className={["fmed-style-dictionary-box", aperto && "fmed-style-dictionary-box-open"].filter(Boolean).join(" ")}>
      <button type="button" onClick={() => setAperto((prev) => !prev)} className="fmed-style-dictionary-accordion-header">
        <div className="fmed-style-dictionary-accordion-left">
          <div className="fmed-style-dictionary-accordion-icon">☰</div>
          <div>
            <h3 className="fmed-style-dictionary-accordion-title">{titolo}</h3>
            <div className="fmed-style-dictionary-accordion-subtitle">{vociCount} voci disponibili</div>
          </div>
        </div>

        <div className="fmed-style-dictionary-accordion-right">
          <span className="fmed-style-dictionary-accordion-badge">{aperto ? "Aperto" : "Chiuso"}</span>
          <span className="fmed-style-dictionary-accordion-chevron">{aperto ? "▲" : "▼"}</span>
        </div>
      </button>

      {aperto && <div className="fmed-style-dictionary-accordion-content">
          <input value={valore || ""} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} onKeyDown={(e) => {
        ...
        marginTop: "12px",
        width: "100%"
      }} onClick={onAdd} className="fmed-style-primary-btn">
             Aggiungi voce
          </button>

          <div className="fmed-style-dictionary-preview">
          ...
          if (inModifica) {
            return <span key={item} className="fmed-style-dictionary-chip fmed-style-dictionary-chip-editing">
              ...
              }} className="fmed-style-dictionary-inline-input" />
                    <button type="button" title="Salva modifica" onClick={(e) => {
                e.stopPropagation();
                onSaveEdit(tipo, item);
              }} className="fmed-style-dictionary-chip-save">
                      ✓
                    </button>
                    <button type="button" title="Annulla modifica" onClick={(e) => {
                e.stopPropagation();
                onCancelEdit();
              }} className="fmed-style-dictionary-chip-cancel">
                      ↩
                    </button>
                  </span>;
          }
          return <span key={item} className="fmed-style-dictionary-chip">
                  <span className="fmed-style-dictionary-chip-text">{item}</span>
                  {onEdit && tipo ? <button type="button" title={`Modifica ${item}`} onClick={(e) => {
              e.stopPropagation();
              onEdit(tipo, item);
            }} className="fmed-style-dictionary-chip-edit">
                      
                    </button> : null}
                  {onRemove && tipo ? <button type="button" title={`Rimuovi ${item}`} onClick={(e) => {
              e.stopPropagation();
              onRemove(tipo, item);
            }} className="fmed-style-dictionary-chip-remove">
                      ×
                    </button> : null}
                </span>;
        }) : <span className="fmed-style-muted-small">Nessuna voce disponibile</span>}
          </div>
        </div>}
    </div>;
}
...
function AppNuovo() {
  ...
  useEffect(() => {
    try {
      ...
      // Il tema chiaro resta comunque disponibile nella sessione corrente.
    }if (!sessioneFmed) {document.body.dataset.theme = "light";document.documentElement.dataset.theme = "light";
    }
  }, [sessioneFmed]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // no-op
    }}, []);const salvaSessioneFmed = useCallback((nuovaSessione) => {rimuoviSessioneFmedSalvata();
  ...
  if (!sessioneFmed) {
    return <div className="fmed-login-page fmed-operational2-login fmed-login-style-page">
        <section className="fmed-login-brand-panel" aria-label="FMED Facility Management Database">
          <div className="fmed-login-brand-content">
            <div className="fmed-login-monogram" aria-hidden="true">F.</div>
            <div className="fmed-login-brand-kicker">F.M.E.D.</div>
            <h1>Facility Management Database</h1>
            <p>Un unico ambiente per governare asset, interventi, scadenze e infrastrutture.</p>
          </div>
          <div className="fmed-login-brand-footer">Facility Management di Fabio Carratù</div>
        </section>

        <section className="fmed-login-form-panel">
          <div className="fmed-login-card fmed-login-style-card">
            <div className="fmed-login-kicker fmed-login-style-kicker">Accesso riservato</div>
            <h1 className="fmed-login-title fmed-login-style-title">Bentornato in FMED</h1>
            <p className="fmed-login-subtitle fmed-login-style-subtitle">Inserisci le credenziali aziendali per accedere all’area tecnica.</p>

            <form className="fmed-login-form fmed-login-style-form" onSubmit={loginSubmit}>
              <label className="fmed-login-style-label">Email</label>
              <input value={loginUsername} onChange={(e) => setLoginUsername(e.target.value)} placeholder="email aziendale" autoComplete="username" className="fmed-login-style-input" />

              <label className="fmed-login-style-label">Password</label>
              <input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} placeholder="password" autoComplete="current-password" className="fmed-login-style-input" />

              <label className="fmed-login-remember fmed-login-style-remember-row">
              ...
              {loginErrore ? <div className="fmed-login-error fmed-login-style-error">{loginErrore}</div> : null}

              <button className="fmed-login-submit fmed-login-style-button" type="submit" disabled={loginLoading}>
                {loginLoading ? "Accesso..." : "Accedi"}
              </button>

              <div className="fmed-login-version fmed-login-style-version-text">{MRDB_APP_BUILD_LABEL}</div>
            </form>
          </div>
        </section>
      </div>;
