...
export default function AssetPage(props) {
  ...
  return <main className={`p0-asset-page ${assetElencoAperto ? "has-results" : ""}`}>
    ...
    <section className="p0-asset-deck">
      ...
      <AssetControls {...props} assetBulkEditorOpen={assetBulkEditorOpen} setAssetBulkEditorOpen={setAssetBulkEditorOpen} assetSelezionatiBulk={assetSelezionatiBulk} />



      {assetElencoAperto && <section className="p0-asset-results" aria-label="Elenco asset filtrati">
        <div className="p0-asset-results-head">
          <div><span>RISULTATI</span></div>
        </div>

        {assetBulkEditorOpen && assetSelezionatiBulk.length > 0 && <div className="p0-asset-bulk">
          <div className="p0-asset-bulk-title"><strong>Modifica multipla</strong><span>{assetSelezionatiBulk.length} asset selezionati · compila solo i campi da modificare</span></div>
          <CanonicalSelect dictionary="BRANCHE_MEDICHE" allowQuickAdd={false} value={assetBulkBranca} onChange={setAssetBulkBranca} options={listaBranche} placeholder="Branca: non modificare" apiBaseUrl={apiBaseUrl} />
          <CanonicalSelect dictionary="SOCIETA" allowQuickAdd={false} value={assetBulkSocieta} onChange={setAssetBulkSocieta} options={listaSocieta} placeholder="Società: non modificare" apiBaseUrl={apiBaseUrl} />
          <CanonicalSelect dictionary="SEDI" allowQuickAdd={false} value={assetBulkSede} onChange={(value) => { setAssetBulkSede(value); setAssetBulkLocazione(""); }} options={listaSedi} placeholder="Sede: non modificare" apiBaseUrl={apiBaseUrl} />
          <CanonicalSelect dictionary="LOCAZIONI" allowQuickAdd={false} value={assetBulkLocazione} disabled={!assetBulkSede} onChange={setAssetBulkLocazione} options={getListaLocazioniPerSede(assetBulkSede || "TUTTE", assetBulkLocazione, true)} placeholder="Locazione: non modificare" apiBaseUrl={apiBaseUrl} restrictToOptions />
          <CanonicalSelect dictionary="STATI_ASSET" allowQuickAdd={false} value={assetBulkStato} onChange={setAssetBulkStato} options={STATI_ASSET_STANDARD} placeholder="Stato: non modificare" apiBaseUrl={apiBaseUrl} />
          <button type="button" className="is-apply" disabled={assetBulkSaving} onClick={salvaModificaMultiplaBrancaAsset}>Applica</button>
          <button type="button" onClick={() => setAssetBulkEditorOpen(false)}>Annulla</button>
        </div>}

        <div className="p0-asset-list">
          <div className="p0-asset-list-labels"><span className="p0-asset-select-spacer" aria-hidden="true"></span><span className="p0-asset-list-spacer" aria-hidden="true"></span><span>Posizione</span><span>Stato</span><span>Azioni</span></div>
          {filtratiRenderizzati.map((asset, index) => {
            ...
            return <article className={`p0-asset-row ${editing ? "is-editing" : ""}`} key={code || index}>
              ...
              <div className="p0-asset-row-actions">
                <button type="button" className="is-open" onClick={() => apriSchedaCespite(asset)}>Apri</button>
                <button type="button" className="is-edit" onClick={() => apriModificaRapidaAsset(asset)}>Modifica</button>
              </div>
