...
export default function InterventiPage(props) {
  const {
    interventiFiltrati, interventi, interventiIncludeStorico,
    cambiaVistaStoricoInterventi, formatCurrency, importoIntervento,
    totaleSpesaInterventiFiltrati, labelPeriodoContabileInterventi,
    codiciCoinvoltiInterventi, ditteCoinvolteInterventi, interventiElencoAperto,
    setInterventiElencoAperto, interventiFiltratiRenderizzati, apriSchedaDaCodice,
    normalizzaSocietaDitta, formattaData, BottoneJobReport, apriModificaIntervento,
    eliminaIntervento, setInterventiRenderLimit, FMED_RENDER_BATCH_INTERVENTI
  } = props;
  ...
  return (
    <main className="p0-operations p0-operations--maintenance">
      ...
      <section className="p0-metric-strip" aria-label="Sintesi interventi">
      ...
      <section className="p0-history-switch">
        <div><b>{interventiIncludeStorico ? "Registro completo" : "Vista operativa"}</b><span>{interventiIncludeStorico ? "Sono inclusi anche i record precedenti al 2023." : "Attività dal 2023 a oggi; i collaudi restano sempre disponibili."}</span></div>
              ...
              <tbody>
                {interventiFiltratiRenderizzati.map((row, index) => (
                  <tr key={row.id_intervento || index}>
                    ...
                    <td><b>{row.attivita || "-"}</b>{row._eccezione_collaudo && <span className="p0-tag">Collaudo conservato</span>}{row._archivio_storico && <span className="p0-tag">Pre-2023</span>}</td>
