              ...
              <tbody>
                {interventiFiltratiRenderizzati.map((row, index) => (
                  <tr key={row.id_intervento || index}>
                    ...
                    <td><b>{row.attivita || "-"}</b>{row.ciclo_chiave && <span className="p0-tag">{row.stato_ciclo || "ATTIVA"} · {row.ciclo_chiave}</span>}{row._eccezione_collaudo && <span className="p0-tag">Collaudo conservato</span>}{row._archivio_storico && <span className="p0-tag">Pre-2023</span>}</td>
