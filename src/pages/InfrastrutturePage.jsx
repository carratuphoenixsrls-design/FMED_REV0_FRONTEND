...
export default function InfrastrutturePage(props) {
  const {
    infrastruttureFiltrate, infrastruttureConStato, infraOk, infraInScadenza,
    infraScadute, formattaData, formInfrastrutturaOpen
  } = props;
  const metrics = [
            ...
            <tbody>
              {infrastruttureFiltrate.map((row,index) => (
                <tr key={row.id || index}>
                  ...
                  <td><span className="p0-state"><i style={{ background: row._statoInfra.colore }} />{row._statoInfra.testo}</span></td>
