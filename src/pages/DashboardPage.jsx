...
export default function DashboardPage({
  ...
  const loadDashboard = useCallback(async ({ force = false } = {}) => {
    ...
    try {
      ...
      const data = await fmedFetchJson(`/process-engine/riepilogo?limit=300&include_storico=${filters.tutto_storico ? "true" : "false"}`, { apiBaseUrl, headers: fmedAuthHeaders(), timeoutMs: 90000, retries: 1 });
      const summary = data?.riepilogo || {};
      const processRows = Array.isArray(data?.processi) ? data.processi : [];
      const normalizedProcesses = processRows.map((row) => ({
        ...row,
        tipo: "PROCESSO",
      ...
      setSnapshot({
        kpi: {
          processi_totali: summary.totale || 0,
          processi_aperti: summary.aperti || 0,
          processi_in_ritardo: summary.in_ritardo || 0,
          processi_da_approvare: summary.da_approvare || 0,
        },
        processi_operativi: normalizedProcesses.filter((row) => !["COMPLETATO", "ANNULLATO", "ERRORE"].includes(String(row?.stato || "").toUpperCase())),
        approvazioni_pendenti: normalizedProcesses.filter((row) => ["DA_RICHIEDERE", "DA_APPROVARE"].includes(String(row?.approvazione_stato || "").toUpperCase())),
      });
  ...
  const kpi = { ...fallbackKpi, ...(snapshot?.kpi || {}) };
  ...
  return <div className={`fmed-dashboard-page fmed-dashboard-dashboard fmed-operational-dashboard ${drilldown ? "is-workspace-open" : ""}`} data-fmed-dashboard="REV0">
    <header className="fmed-dashboard-header fmed-operational-header">
      ...
      <div className="fmed-dashboard-header-actions">
        <span className="fmed-dashboard-live" role="status" aria-live="polite"><i /> {loading ? "Aggiornamento in corso…" : "Dati aggiornati"}</span>
    ...
    <section className="fmed-operational-automation-strip">
      <div><strong>Automazioni attive</strong>{" "}<span>Scadenze, cicli, storico, codici e dashboard si aggiornano automaticamente quando registri o chiudi un’attivita' .</span></div>
