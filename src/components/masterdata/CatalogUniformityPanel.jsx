...
export default function CatalogUniformityPanel({
  ...
  return <section className="core-uniformity-panel" aria-label="Uniformità cataloghi Supabase">
    <div className="core-quality-heading">
      ...
      <button type="button" className="core-primary-button" onClick={onRefresh} disabled={loading || busy}>{loading ? "Analisi Supabase…" : "1 · Analizza tutti i cataloghi"}</button>
    </div>
    ...
    <div className="core-uniformity-governance">
      <article><b>Prima: pulizia controllata</b><span>Analisi completa, anteprima dei duplicati esatti e applicazione solo dopo conferma Admin.</span></article>
      <article><b>Dopo: catalogazione unica</b><span>I moduli propongono esclusivamente valori canonici; le varianti storiche continuano a essere riconosciute come alias.</span></article>
      <article><b>Nuove voci: niente testo libero</b><span>Le nuove denominazioni passano dal pulsante ＋ o dalla pagina Cataloghi, con codice automatico e controllo delle somiglianze.</span></article>
    </div>
    ...
    {loading && !audit ? <div className="core-quality-loading">Lettura dei dizionari, dei valori, delle relazioni e degli utilizzi operativi…</div> : <>
      ...
      <div className="core-quality-actions core-uniformity-action">
        ...
        <button type="button" onClick={onPreviewExact} disabled={busy || !exact.length}>2 · Prepara anteprima</button>
        {canManage && mergePreview && <button type="button" className="core-danger-safe-button" onClick={onApplyExact} disabled={busy || !exactPlan.length}>3 · Unifica {exactPlan.length} gruppi esatti</button>}
      </div>
