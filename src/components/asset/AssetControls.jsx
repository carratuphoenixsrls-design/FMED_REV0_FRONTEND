...
export default function AssetControls(props) {
  const {
    ...
    assetBulkEditorOpen = false,
    setAssetBulkEditorOpen = () => {},
  ...
  return <section className="p0-asset-search" aria-label="Ricerca e filtri asset">
    ...
    <div className="p0-asset-commandbar">
      <button type="button" className="is-primary is-open-list" onClick={() => setAssetElencoAperto(true)}>Apri elenco filtrato</button>
      <button type="button" className="is-analysis" onClick={() => setAssetAnalisiAperta(true)}>Analizza asset</button>
      <button type="button" className="is-new" onClick={() => { setCodiceCespiteAutomatico(true); avviaProcessoGuidatoFmed("NUOVO_ASSET"); }}>Nuovo asset</button>
      <button type="button" className="is-master" onClick={() => { setImpostazioniTab("MASTER_DATA"); setPagina("Dizionari"); }}>Master Data</button>
      <button type="button" className="is-quiet is-reset" onClick={resetFilters}>Azzera filtri</button>
      {assetElencoAperto && <>
        <button type="button" className="is-select-all" onClick={selezionaTuttiAssetFiltratiBulk}>Seleziona tutti</button>
        <button type="button" className="is-bulk-edit" disabled={assetSelezionatiBulk.length === 0} onClick={() => setAssetBulkEditorOpen(true)}>Modifica multipla asset</button>
        <button type="button" className="is-close-results" onClick={() => { setAssetBulkEditorOpen(false); setAssetElencoAperto(false); }}>Chiudi elenco</button>
      </>}
